...
byte analogValues[1][7]; // [0도/180도][채널0~3]
int num = 8;  //4개라면 for문 때문에 1빼줌
int analog_feedback[7]; //
...
void pca9685_setting(){
  ...
  // 0도 위치로 이동
  int degree2pul_0 = map(0, 0, 180, 150, 600); // 0~180도 값을 150~600 PWM 값으로 변환
  ...
  // 180도 위치로 이동
  int degree2pul_180 = map(178, 0, 180, 150, 600); // 0~180도 값을 150~600 PWM 값으로 변환
  for (int ch = 0; ch<num; ch++){
  ...
  for (int ch = 0; ch < num; ch++) {
    ...
    Serial.print(ch); Serial.println(analogValues[1][ch]);
  }
  delay(10); // 측정 후 대기
...
const int muxChannel[16][4] PROGMEM={ {0,0,0,0},  
      {1,0,0,0}, //channel 1 
      {0,1,0,0}, //channel 2 
      {1,1,0,0}, //channel 3 
      {0,0,1,0}, //channel 4 
